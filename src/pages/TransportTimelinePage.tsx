  ...
  return (
    <div className="min-h-screen bg-paper dark:bg-ink-950 pt-20 pb-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <RevealOnScroll>
          <SectionHeader
            label="ANCIENT TRANSPORT"
            title="古代交通时间地图"
            description="从商代到清代，探索古人交通智慧与路径规划"
          />
        </RevealOnScroll>
          ...
          <div className="bg-white dark:bg-ink-900 rounded-2xl border-2 border-ink-200 dark:border-ink-700 p-6 shadow-lg mb-8">
            <div className="flex flex-wrap gap-4">
              <div className="flex-1 min-w-[200px]">
                <label className="block text-sm font-medium text-ink-600 dark:text-ink-400 mb-2">🏛️ 朝代时期</label>
                <select
                  value={selectedEra}
              ...
              <div className="flex-1 min-w-[200px]">
                <label className="block text-sm font-medium text-ink-600 dark:text-ink-400 mb-2">🛣️ 交通路线</label>
                <select
                  value={selectedRoute}
        ...
        <RevealOnScroll delay={200}>
          <div className="bg-gradient-to-br from-paper to-ink-50 dark:from-ink-900 dark:to-ink-800 rounded-2xl border-2 border-ink-200 dark:border-ink-700 p-6 md:p-8 shadow-lg mb-8">
            <h3 className="text-xl font-bold text-ink-900 dark:text-ink-100 mb-6 flex items-center gap-2">
              <span className="text-2xl">🧭</span> AI 路径引擎 — 起终点 + 朝代 → 最短路径
            </h3>

            {/* 输入区 */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div>
                <label className="block text-sm font-medium text-ink-600 dark:text-ink-400 mb-1">起点</label>
                <select
                  value={startCity}
              ...
              <div>
                <label className="block text-sm font-medium text-ink-600 dark:text-ink-400 mb-1">终点</label>
                <select
                  value={endCity}
              ...
              <div>
                <label className="block text-sm font-medium text-ink-600 dark:text-ink-400 mb-1">朝代</label>
                <select
                ...
                >
                  {ALL_DYNASTIES.map(d => (
                    <option key={d} value={d}>{d}（速度系数 {DYNASTY_SPEED_FACTORS[d]}）</option>
                  ))}
                </select>
              </div>
            ...
            {pathResult && (
              <div className="p-4 rounded-xl bg-white dark:bg-ink-900 border-2 border-accent/30 mb-4">
                <h4 className="font-bold text-accent mb-2">📍 最短路径（{selectedDynasty}朝）</h4>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <div className="text-sm">
                    ...
                    <span className="font-bold text-ink-900 dark:text-ink-100">{pathResult.totalDays} 天</span>
                  </div>
                  <div className="text-sm">
                    <span className="text-ink-500">距离：</span>
                    <span className="font-bold text-ink-900 dark:text-ink-100">{pathResult.totalDistanceKm} km</span>
                  </div>
                  <div className="text-sm col-span-2 md:col-span-1">
                    <span className="text-ink-500">速度系数：</span>
            ...
            {dynastyComparison.length > 0 && (
              <div className="p-4 rounded-xl bg-white dark:bg-ink-900 border-2 border-indigo-200 dark:border-indigo-800 mb-4">
                <h4 className="font-bold text-indigo-700 dark:text-indigo-400 mb-3">⚖️ 多朝代天数对比</h4>
                  ...
                  {dynastyComparison.map((r, i) => (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 bg-ink-200 dark:bg-ink-700 rounded-full h-4">
                        ...
                        >
                          <span className="text-xs text-white font-bold">{r.totalDays}天</span>
                        </div>
                      </div>
            ...
            {reachableCities.length > 0 && (
              <div className="mt-3 p-4 rounded-xl bg-green-50 dark:bg-green-900/20 border-2 border-green-200 dark:border-green-800">
                <h4 className="font-bold text-green-800 dark:text-green-400 mb-2">
                  🗺️ 从 {startCity} 出发，{selectedDynasty}朝 30天内可达：
                </h4>
                <div className="flex flex-wrap gap-2">
                  {reachableCities.map((c, i) => (
                    <span key={i} className="px-3 py-1 rounded-full bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 text-sm">
                      {c.city}（{c.days}天，{c.distanceKm}km）
                    </span>
                  ))}
                </div>
              </div>
        ...
        <RevealOnScroll>
          <div className="mb-12">
            <SectionHeader label="TRANSPORT ROUTES" title="🛣️ 重要交通路线" description="连接中华文明的生命线" />
        ...
        <RevealOnScroll>
          <div className="mb-12">
            <SectionHeader label="ERA TIMELINE" title="📊 交通发展时间线" description="从商代到清代的交通演变" />
        ...
        <div className="bg-gradient-to-r from-orange-500 to-amber-500 p-8 rounded-t-3xl">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/90">{isRoute ? '交通路线' : '交通发展'}</p>
            </div>
          </div>
        </div>
              ...
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-ink-50 dark:bg-ink-800 rounded-xl p-4">
                    <h4 className="font-bold text-ink-900 dark:text-ink-100 mb-2">起点 → 终点</h4>
                  ...
                  <div className="bg-ink-50 dark:bg-ink-800 rounded-xl p-4">
                    <h4 className="font-bold text-ink-900 dark:text-ink-100 mb-2">通行时间</h4>
                ...
                <div className="bg-ink-50 dark:bg-ink-800 rounded-xl p-4">
                  <h4 className="font-bold text-ink-900 dark:text-ink-100 mb-2">📍 主要站点</h4>
                  <div className="flex flex-wrap gap-2">
                ...
                <div className="bg-gradient-to-r from-orange-50 to-amber-50 dark:from-orange-900/20 dark:to-amber-900/20 rounded-xl p-4">
                  <h4 className="font-bold text-ink-900 dark:text-ink-100 mb-2">⏰ 时间线</h4>
          ...
          {!isRoute && (() => {
            ...
            return (
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-ink-50 dark:bg-ink-800 rounded-xl p-4">
                    <h4 className="font-bold text-ink-900 dark:text-ink-100 mb-2">🚗 交通方式</h4>
                  ...
                  <div className="bg-ink-50 dark:bg-ink-800 rounded-xl p-4">
                    <h4 className="font-bold text-ink-900 dark:text-ink-100 mb-2">🏗️ 基础设施</h4>
                    <div className="flex flex-wrap gap-1">
                ...
                <div>
                  <h4 className="font-bold text-ink-900 dark:text-ink-100 mb-2">🎉 重要事件</h4>
