  ...
  return (
    <div className="p-4 rounded-xl bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-700">
      ...
      <div className="flex items-center gap-2 mb-2">
        <span className="text-xs text-ink-500">{t('futurePrediction.probability_label')}</span>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-4 rounded-xl bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-700">
          ...
          <div className="text-sm text-ink-500">{t('futurePrediction.stats_total')}</div>
        </div>
        <div className="p-4 rounded-xl bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-700">
          <div className="text-2xl font-bold text-green-600">{stats.correct}</div>
          <div className="text-sm text-ink-500">{t('futurePrediction.stats_correct')}</div>
        </div>
        <div className="p-4 rounded-xl bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-700">
          <div className="text-2xl font-bold text-yellow-600">{stats.partial}</div>
          <div className="text-sm text-ink-500">{t('futurePrediction.stats_partial')}</div>
        </div>
        <div className="p-4 rounded-xl bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-700">
          <div className="text-2xl font-bold text-accent">{stats.accuracy}%</div>
          <div className="text-sm text-ink-500">{t('futurePrediction.stats_accuracy')}</div>
        </div>
      </div>
      ...
      {predictions.length === 0 ? (
        <div className="text-center py-12 text-ink-400">
          <p>{t('futurePrediction.no_predictions')}</p>
        </div>
      ) : (
            ...
            <div key={pred.id} className="p-4 rounded-xl bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-700">
              <div className="flex items-center justify-between mb-2">
                ...
                {pred.result && (
                  <span className={`text-xs px-2 py-1 rounded ${
                    ...
                    {pred.result === 'correct' ? t('futurePrediction.result_correct') : pred.result === 'partial' ? t('futurePrediction.result_partial') : t('futurePrediction.result_wrong')}
                  </span>
                )}
              </div>
              <p className="text-sm text-ink-600 dark:text-ink-400 mb-2">{pred.prediction}</p>
              <div className="flex items-center justify-between text-xs text-ink-500">
                <span>{t('futurePrediction.confidence_display', { value: pred.confidence })}</span>
              ...
              {!pred.result && (
                <div className="flex gap-2 mt-3">
                  ...
                  >
                    {t('futurePrediction.mark_correct')}
                  </button>
                  <button
...
export default function FuturePredictionPage() {
  ...
  const handleForecast = useCallback(async () => {
    if (selectedEvents.length === 0) {
      alert(t('futurePrediction.alert_select_event'));
      return;
    }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : t('futurePrediction.alert_forecast_error_default');
      alert(t('futurePrediction.alert_forecast_failed', { message }));
    } finally {
  ...
  const handleSavePrediction = useCallback(() => {
    if (!userPredictionInput.trim()) {
      alert(t('futurePrediction.alert_input_prediction'));
      return;
    }
    ...
    setConfidence(50);
    alert(t('futurePrediction.alert_saved'));
  ...
  return (
    <div className="min-h-screen bg-paper dark:bg-ink-950 pt-20 pb-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <RevealOnScroll>
          <SectionHeader
            label={t('futurePrediction.label')}
            title={t('futurePrediction.title')}
            description={t('futurePrediction.description')}
          />
        </RevealOnScroll>
        ...
        <RevealOnScroll delay={100}>
          <div className="flex gap-2 mt-6 mb-8 overflow-x-auto pb-2">
            {[
              { id: 'timeline' as const, labelKey: 'futurePrediction.tab_timeline', emoji: '' },
              { id: 'predict' as const, labelKey: 'futurePrediction.tab_predict', emoji: '' },
              { id: 'my-predictions' as const, labelKey: 'futurePrediction.tab_my_predictions', emoji: '' },
              { id: 'scenario' as const, labelKey: 'futurePrediction.tab_scenario', emoji: '' },
            ].map(tab => (
              <button
                key={tab.id}
          ...
          <RevealOnScroll direction="up" delay={200}>
            <div className="bg-white dark:bg-ink-900 rounded-2xl border-2 border-ink-200 dark:border-ink-700 p-6 shadow-lg">
              <h3 className="text-lg font-bold text-ink-900 dark:text-ink-100 mb-4">
                {t('futurePrediction.historical_events_title')}
              </h3>
              ...
              {selectedEvents.length > 0 && (
                <div className="mt-4 p-3 rounded-xl bg-accent/10 text-accent font-bold text-sm">
                  {t('futurePrediction.selected_events', { count: selectedEvents.length, events: selectedEvents.map(e => e.displayName).join('、') })}
                </div>
              )}
            </div>
          </RevealOnScroll>
        ...
        {activeTab === 'predict' && (
          <RevealOnScroll direction="up" delay={200}>
            <div className="space-y-6">
              ...
              <div className="bg-white dark:bg-ink-900 rounded-2xl border-2 border-ink-200 dark:border-ink-700 p-6 shadow-lg">
                <h3 className="text-lg font-bold text-ink-900 dark:text-ink-100 mb-4">
                  {t('futurePrediction.future_events_title')}
                </h3>
              ...
              <div className="bg-white dark:bg-ink-900 rounded-2xl border-2 border-ink-200 dark:border-ink-700 p-6 shadow-lg">
                <h3 className="text-lg font-bold text-ink-900 dark:text-ink-100 mb-4">
                  {t('futurePrediction.prediction_based_on_history')}
                </h3>

                {/* 已选事件 */}
                <div className="mb-4">
                  <p className="text-sm text-ink-500 mb-2">{t('futurePrediction.reference_events')}</p>
                  {selectedEvents.length === 0 ? (
                    <p className="text-sm text-ink-400 italic">{t('futurePrediction.reference_events_empty')}</p>
                  ) : (
                ...
                {/* 用户预测输入 */}
                <div className="mb-4">
                  <label className="text-sm text-ink-500 mb-2 block">{t('futurePrediction.your_prediction')}</label>
                  <textarea
                    value={userPredictionInput}
                    onChange={(e) => setUserPredictionInput(e.target.value)}
                    className="input-field w-full h-24"
                    placeholder={t('futurePrediction.prediction_placeholder')}
                  />
                </div>
                ...
                <div className="mb-6">
                  <label className="text-sm text-ink-500 mb-2 block">
                    {t('futurePrediction.confidence_label', { value: confidence })}
                  </label>
                  <input
                    type="range"
                    min={0}
                  ...
                  <div className="flex justify-between text-xs text-ink-400">
                    <span>{t('futurePrediction.confidence_low')}</span>
                    <span>{t('futurePrediction.confidence_high')}</span>
                  </div>
                </div>
                ...
                <div className="flex gap-3 flex-wrap">
                  ...
                  >
                    {isForecasting ? t('futurePrediction.forecasting') : t('futurePrediction.ai_forecast')}
                  </button>
                  <button
              ...
              {llmForecast && (
                <div className="bg-gradient-to-br from-paper to-ink-50 dark:from-ink-900 dark:to-ink-800 rounded-2xl border-2 border-ink-200 dark:border-ink-700 p-6 shadow-lg">
                  <h3 className="text-lg font-bold text-accent mb-4 text-center">{t('futurePrediction.ai_result_title')}</h3>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div className="p-4 rounded-xl bg-white dark:bg-ink-900">
                      <div className="text-sm text-ink-500 mb-1">{t('futurePrediction.conclusion_label')}</div>
                      <p className="text-sm text-ink-700 dark:text-ink-300">{llmForecast.conclusion}</p>
                    </div>
                    <div className="p-4 rounded-xl bg-white dark:bg-ink-900">
                      <div className="text-sm text-ink-500 mb-1">{t('futurePrediction.probability_label')}</div>
                      ...
                      <div className="text-sm text-ink-500 mt-2">{t('futurePrediction.time_window_label', { window: llmForecast.timeWindow })}</div>
                    </div>
                  </div>

                  {llmForecast.keyVariables.length > 0 && (
                    <div className="mb-4">
                      <h4 className="font-bold text-sm text-ink-700 dark:text-ink-300 mb-2">{t('futurePrediction.key_variables')}</h4>
                      <div className="flex flex-wrap gap-2">
                  ...
                  {llmForecast.caveats.length > 0 && (
                    <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                      <h4 className="font-bold text-sm text-amber-800 dark:text-amber-400 mb-1">{t('futurePrediction.uncertainty')}</h4>
                  ...
                  {userPredictionInput && (
                    ...
                    >
                      {isComparing ? t('futurePrediction.comparing') : t('futurePrediction.compare_btn')}
                    </button>
                  )}
                  ...
                  {comparison && (
                    <div className="mt-4 space-y-4">
                      <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-200 dark:border-blue-800">
                        <h4 className="font-bold text-blue-800 dark:text-blue-400 mb-2">{t('futurePrediction.evaluation_title')}</h4>
                      ...
                      <div className="p-4 rounded-xl bg-green-50 dark:bg-green-900/20 border-2 border-green-200 dark:border-green-800">
                        <h4 className="font-bold text-green-800 dark:text-green-400 mb-2">{t('futurePrediction.alignment_title')}</h4>
                        <p className="text-sm text-ink-700 dark:text-ink-300">
                          {comparison.alignment === 'aligned' ? t('futurePrediction.aligned') :
                           comparison.alignment === 'divergent' ? t('futurePrediction.divergent') :
                           t('futurePrediction.complementary')}
                        </p>
                      </div>
                      {comparison.insight && (
                        <div className="p-4 rounded-xl bg-purple-50 dark:bg-purple-900/20 border-2 border-purple-200 dark:border-purple-800">
                          <h4 className="font-bold text-purple-800 dark:text-purple-400 mb-2">{t('futurePrediction.insight_title')}</h4>
        ...
        {activeTab === 'my-predictions' && (
          <RevealOnScroll direction="up" delay={200}>
            <div className="bg-white dark:bg-ink-900 rounded-2xl border-2 border-ink-200 dark:border-ink-700 p-6 shadow-lg">
              <h3 className="text-lg font-bold text-ink-900 dark:text-ink-100 mb-6">{t('futurePrediction.my_predictions_title')}</h3>
        ...
        {activeTab === 'scenario' && (
          <RevealOnScroll direction="up" delay={200}>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {/* 场景列表 */}
              <div className="bg-white dark:bg-ink-900 rounded-2xl border-2 border-ink-200 dark:border-ink-700 p-6 shadow-lg">
                <h3 className="text-lg font-bold text-ink-900 dark:text-ink-100 mb-4">{t('futurePrediction.scenarios_title')}</h3>
                ...
                {selectedScenario ? (
                  <div>
                    ...
                    <div className="p-4 rounded-xl bg-ink-50 dark:bg-ink-800 mb-4">
                      <h4 className="font-bold text-sm text-ink-700 dark:text-ink-300 mb-1">{t('futurePrediction.scenario_starting_point')}</h4>
                      <p className="text-sm text-ink-600 dark:text-ink-400">{selectedScenario.startingPoint}</p>
                    </div>

                    {/* 变量 */}
                    <h4 className="font-bold text-ink-900 dark:text-ink-100 mb-3">{t('futurePrediction.scenario_variables')}</h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
                      {selectedScenario.variables.map(variable => (
                        <div key={variable.id} className="p-3 rounded-xl bg-ink-50 dark:bg-ink-800">
                          ...
                          <p className="text-sm text-accent font-bold mt-2">{t('futurePrediction.variable_current_value', { value: variable.currentValue })}</p>
                        </div>
                      ))}
                    </div>

                    {/* 分支结果 */}
                    <h4 className="font-bold text-ink-900 dark:text-ink-100 mb-3">{t('futurePrediction.scenario_results')}</h4>
                  ...
                  <div className="text-center text-ink-400 py-20">
                    <p>{t('futurePrediction.scenario_select_prompt')}</p>
                  </div>
                )}
              </div>
            </div>
