    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-paper dark:bg-ink-950">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-ink-800 dark:text-ink-200 mb-2">{t('topicDetail.not_found_title')}</h2>
      ...
      <div className="sticky top-0 z-40 bg-white/90 dark:bg-ink-900/90 backdrop-blur-sm border-b border-ink-200 dark:border-ink-700">
        <div className="max-w-4xl mx-auto px-6 py-3 flex items-center justify-between">
          <button onClick={() => navigate('/topics')} className="text-sm text-ink-500 hover:text-accent transition-colors">
            ← {t('topicDetail.back_to_topics')}
          </button>
          <div className="flex items-center gap-3">
            <span className="text-xs text-ink-400 dark:text-ink-500">
              {t('topicDetail.progress_complete', { n: progressPercent })}
            </span>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-4 text-xs text-ink-400 dark:text-ink-500">
            <span>{t('topicDetail.chapter_count', { n: topic.chapterCount })}</span>
            <span>·</span>
            <span>{t('topicDetail.minutes_count', { n: topic.estimatedMinutes })}</span>
            <span>·</span>
            <span>{topic.tags.map(tag => `#${tag}`).join(' ')}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <aside className="md:w-64 flex-shrink-0">
            <div className="sticky top-20 bg-white dark:bg-ink-900 rounded-xl shadow-sm border border-ink-100 dark:border-ink-700 p-4">
              <h3 className="text-sm font-bold text-ink-700 dark:text-ink-300 mb-3">{t('topicDetail.chapter_directory')}</h3>
              ...
              {topic.references.length > 0 && (
                <div className="mt-6 pt-4 border-t border-ink-100 dark:border-ink-700">
                  <h4 className="text-xs font-bold text-ink-500 dark:text-ink-400 mb-2">{t('topicDetail.references')}</h4>
