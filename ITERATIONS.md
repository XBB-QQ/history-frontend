...
### 一、背景
C 方案完成后，用户提议「显示古地名会好点，鼠标选中才显示当今的地名」。评估后认可：贴合「五千年史馆」主题氛围，更有沉浸感和教学价值，且 `region.aliases` 字段已有古名数据。给三个古名选择策略（A 简单取 aliases[0] / B 智能匹配朝代 / C 折中分 4 组）+ 三个显示规则（1 高亮显示 / 2 全省默认 / 3 高亮显示+非高亮无字），用户选 B + 2。

### 二、方案详情

#### 1. 朝代 → region 古名映射表（[`map-data.ts`](src/data/core/map-data.ts)）
新增 `dynastyRegionAlias: Record<string, Record<string, string>>`，覆盖 13 个朝代各自高亮的省份：

| 朝代 | 高亮省份数 | 古名示例 |
|------|-----------|---------|
| 夏 | 3 | 河南=豫州 / 山西=冀州 / 陕西=雍州 |
| 商 | 4 | 河南=殷 / 山东=东夷 / 河北=北殷 / 安徽=淮夷 |
| 周 | 6 | 陕西=关中 / 河南=洛邑 / 山西=晋 / 湖北=荆楚 |
| 秦 | 7 | 陕西=内史 / 甘肃=陇西 / 山西=河东 / 四川=巴蜀 |
| 西汉 | 11 | 陕西=三辅 / 河南=司隶 / 山西=并州 / 四川=益州 / 河北=冀州 |
| 东汉 | 13 | 同西汉 + 江西=豫章 |
| 南北朝 | 15 | 江苏=南徐州 / 安徽=南豫州 / 湖北=郢州 / 浙江=东扬州 |
| 唐 | 16 | 陕西=关内道 / 甘肃=陇右道 / 新疆=安西都护府 / 四川=剑南道 / 广东=岭南道 |
| 五代十国 | 15 | 河南=京畿 / 江苏=吴 / 浙江=吴越 / 福建=闽 / 广东=南汉 / 江西=南唐 |
| 宋 | 15 | 河南=京畿路 / 山西=河东路 / 广东=广南东路 / 广西=广南西路 / 江苏=两浙路 |
| 元 | 26 | 黑龙江=辽阳行省 / 新疆=察合台 / 西藏=宣政院 / 台湾=琉球 |
| 明 | 24 | 黑龙江=奴儿干都司 / 河北=北直隶 / 江苏=南直隶 / 台湾=东番 |
| 清 | 26 | 黑龙江=黑龙江将军 / 新疆=新疆省 / 西藏=西藏 / 台湾=台湾省 |

新增辅助函数 `getRegionAlias(dynasty, regionId, fallbackAliases, fallbackName)`：
- 优先查 `dynastyRegionAlias[朝代][regionId]`
- fallback 1：`region.aliases[0]`（如陕西默认 fallback "秦"）
- fallback 2：`region.name`（现代省名）

#### 2. MapSVG 显示规则（[`MapSVG.tsx`](src/components/map/MapSVG.tsx)）
- **所有省份默认显示古名**：高亮区域大字粗体（fontSize 12，opacity 0.95），非高亮区域小字暗色（fontSize 9，opacity 0.55），hover 时非高亮区域提亮（opacity 0.85）
- **hover 气泡改双行对照**：
  - 第一行：古名（粗体，朝代色）
  - 第二行：「今 · 现代省名」（小字浅色）
  - 若古名 = 现代名（fallback 到 region.name 时），气泡降级为单行
  - 气泡下方加小三角指向省份中心
- `hoverRegion` state 新增 `ancientName` 字段

#### 3. 朝代切换时古名自动更新
因为 `ancientName` 通过 `getRegionAlias(selectedDynastyName, ...)` 计算，切换朝代时所有省份古名自动更新。比如陕西在秦朝显示「内史」，唐朝显示「关内道」，明朝显示「陕西布政司」，清朝显示「陕西省」。
...
### 四、教训
1. **数据已有则优先用数据**：`region.aliases` 字段早就存在但只用在不重要的搜索匹配，现在终于被用于显示。这是「先看数据再写代码」的好处
2. **fallback 链设计**：朝代映射表 → aliases[0] → region.name 三级 fallback，保证任何情况下都有显示内容，不会出现空白
3. **气泡降级策略**：当古名=现代名（fallback 到 region.name 的省份，如某些朝代未覆盖的区域），气泡降级为单行避免冗余显示
4. **古名智能匹配的代价**：13 朝 × ~12 省 ≈ 150 条数据，工作量可控；若按 16 朝 × 28 省 = 448 条则代价过大。**只覆盖 dynastyMapData 中实际高亮的 regionIds** 是关键优化
5. **小字暗色避免地图拥挤**：非高亮区域 fontSize 9 + opacity 0.55，既保留信息又不会喧宾夺主；hover 时提亮到 0.85 给即时反馈
